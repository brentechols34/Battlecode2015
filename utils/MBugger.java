...
public class MBugger {
	...
	public MBugger(RobotController rc, Path p) {
		map = new boolean[rc.getMapWidth()][rc.getMapHeight()];
		for (boolean[] i : map)
			Arrays.fill(i, true);
	...
	private boolean isTraversable(int x, int y) {
		try {
			...
			if (rc.senseObjectAtLocation(next) != null) {
				// traversable but currently can not go there object in way
				return false;
			}

			// if it is an obstacle add obstacle and update map
			if (!rc.senseTerrainTile(next).isTraversableAtHeight(
					rc.getRobot().getRobotLevel())) {
