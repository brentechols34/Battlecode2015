...
public class PathMove {
	 ...
	 * You should de-compress those by calling SimplePather.decompress
	...
	public void attemptMove() throws GameActionException {
		...
		if (currentNode>=path.length) {
			rc.setIndicatorString(0, "I appear to have made it.");
			Move.tryMove(myLoc.directionTo(path[path.length - 1]));
			return;
		}
		if (impassable(myLoc.add(myLoc.directionTo(path[currentNode])))) {
